		...
		if err != nil {
			...
			if err != nil {
				...
				if err != nil {
					...
					if respType == response.CHAT_MESSAGE || respType == response.SYSTEM_ANNOUNCEMENT || respType == response.ROUND_START || respType == response.ROUND_END || respType == response.NEXT_PROBLEM || respType == response.ROOM_EXPIRED {
