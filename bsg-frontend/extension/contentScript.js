(function () {
  ...
  fetch("https://leetcode.com/graphql", {
    ...
    .then(res => res.json())
    .then(data => {
      console.log(data.data.question.codeSnippets);
    })
    .catch(console.error);


  const BLOCKED_TABS_IDS = [
    "editorial_tab",
    "solutions_tab",
    "submissions_tab"
  ];
  ...
  const hideTabs = () => {
    BLOCKED_TABS_IDS.forEach((tabId) => {
      const label = document.getElementById(tabId);
      if (!label) return;

      const tabButton = label.closest('.flexlayout__tab_button');
      if (!tabButton) return;

      tabButton.style.display = 'none';
      /* lock instead of hide
      tabButton.style.pointerEvents = "none";
      tabButton.style.opacity = "0.4";
      tabButton.title = "Locked while in a room";
      */
    });
  };

  // reblock when DOM updates
