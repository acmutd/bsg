...
export const TopicComponent = ({topic, toggle}: { topic: Topic; toggle: () => void }) => {
    ...
    return (
        <button
            ...
            ${topic.isSelected ? 'bg-primary text-white border-primary' : 'bg-bsg-surface-input hover:opacity-75'}`}
            onClick={toggle}
        >
            <span className="text-sm font-medium text-foreground">{topic.name}</span>
            <span
                className={`px-2 py-0.5 text-xs font-medium rounded-full 
                ${topic.isSelected ? 'bg-white text-primary' : 'bg-foreground/10 text-foreground/60 opacity-75'}`}
            >
